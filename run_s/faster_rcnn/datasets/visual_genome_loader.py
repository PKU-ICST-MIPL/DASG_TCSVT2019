...
class visual_genome(data.Dataset):
    ...
    def getOneImage(self,imagePath):
        ...
        gt_boxes_object = torch.Tensor(4,5)
        gt_relationships = torch.Tensor(6,5)
        gt_boxes_region = torch.Tensor(4,15)

        img = img.unsqueeze(0)
        im_info = im_info.reshape(1,3)
    ...
    def __getitem__(self, index):
        ...
        imagePath = osp.join(cfg.IMG_DATA_DIR, self.annotations[index]['path'])
        ...
        return img, im_info, gt_boxes_object, gt_relationships, gt_boxes_region, imagePath
