...
def get_transform(data_name, split_name, opt):
    ...
    t_list = []
    if split_name == 'train':
        t_list = [transforms.RandomSizedCrop(opt.crop_size), transforms.RandomHorizontalFlip()]
    elif split_name == 'val':
...
def get_loaders(data_name, vocab, crop_size, batch_size, workers, opt):
    ...
    if opt.data_name.endswith('_precomp'):
        ...
        val_loader = get_precomp_loader(dpath, 'test', vocab, opt, batch_size, False, workers)
    else:
